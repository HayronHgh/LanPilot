...
C++23／CMake，不需要自架轉發服務。最新開發主線將畫面與控制分成兩條
TLS TCP socket，Agent 維持 SSH；舊版 SSH 桌面路徑保留。

## 版本

| 版本線 | 畫面路徑 | 定位 |
| --- | --- | --- |
| v0.1 | H.264 + SSH | 第一條版本線 |
| v0.2 | 自研 RECT + SSH | 第二條版本線，保留 H.264 能力 |
| v0.3 | 自研 RECT + 分離通道 | 最新開發主線：畫面 TLS、控制 TLS、Agent SSH |

**尚無可下載的正式版本。** RECT 的局部量測有改善，但未證明所有工作負載都更快。
發布條件見 [發布清單](doc/public-releases.md)。

版本線是發布規劃，不代表三個正式標籤或安裝包已存在。公開 Git 保留後續
可審查的 commit，不逐一發布開發快照，也不匯入私人研究歷史。
目前進度與最短發布路徑見 [發布就緒報告](doc/release-readiness.md)。

## 安裝兩端

以下是 **v0.1／v0.2 SSH 預覽安裝方式**，不是 v0.3 一鍵 TLS 配對流程。
TLS 身份建立與使用者服務目前仍是工程工具；不要把開發者的設定、憑證或私鑰
複製給其他使用者。以下命令適用於打包後的目錄。發布前，請依 [封裝文件](doc/public-guide.md)
